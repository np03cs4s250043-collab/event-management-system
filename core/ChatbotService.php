...
class ChatbotService {
    ...
    private function buildSystemPrompt(?int $userId, ?string $role): string {
        ...
        return "You are the Eventify assistant for an event booking website. Today is $today.\n\n"
            ...
            . "  Step 1 — Identify the event: use search_events if needed, then confirm the event with the user.\n"
            . "  Step 2 — Ask for quantity: ALWAYS ask 'How many tickets would you like?' before calling propose_booking, unless the user already stated a number.\n"
            . "  Step 3 — Propose: call propose_booking(event_id, quantity) and show the summary (title, date, venue, quantity, total price).\n"
            . "  Step 4 — Wait for confirmation: ask 'Shall I confirm this booking?' Do NOT call create_booking yet.\n"
            . "  Step 5 — Book: call create_booking ONLY when the user replies with 'yes', 'confirm', 'book it', 'book', 'confirmed', or any clear confirmation. If the message contains '(confirmed)' it is a definitive confirmation — call create_booking immediately.\n\n"
            . "RULES:\n"
            . "  1. Always use tools for real data — never invent event names, prices, dates, or booking refs.\n"
            . "  2. Never skip the quantity question. Never call propose_booking without a quantity the user has provided or agreed to.\n"
